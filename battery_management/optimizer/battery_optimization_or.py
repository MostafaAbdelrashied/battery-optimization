...
class FleetOptimizationOR(FleetOptimizationBaseclass):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.spiky_behaviour_penalty = 0.01  # cf. description of flag penalize_spiky_behaviour in baseclass. this
    ...
    def _initialize_grid_power(self):
        ...
        for t in range(self.n_t):
            ...
            if not self.allow_curtailment:
                self.model.Add(self.grid_bool['curtail'][t] == 0, f"disable_curtailment_{t}")
    ...
    def _initialize_battery(self, battery: Battery):
        ...
        for t in range(self.n_t):
            ...
            self.fleet_power[i]["charge"][t] = self.model.NumVar(0, battery.power_charge_max
            if battery.connected[t] else 0, f"power_{i}_charge_{t}")

            self.fleet_power[i]["discharge"][t] = self.model.NumVar(0, battery.power_discharge_max
            if battery.connected[t] else 0, f"power_{i}_discharge_{t}")

            self.fleet_energy[i][t] = self.model.NumVar(battery.energy_min, battery.energy_max, f"energy_{i}_{t}")
            ...
            self.fleet_bool[i]["discharge"][t] = self.model.BoolVar(f"bool_{i}_discharge_{t}")
            self.fleet_bool[i]["disconnected"][t] = self.model.BoolVar(f"bool_{i}_disconnected_{t}")
        ...
        self.fleet_energy[i][0] = battery.energy_start \
                                  + self.fleet_power[i]["charge"][0] * self.dt \
                                  - self.fleet_power[i]["discharge"][0] * self.dt
        self.model.Add(self.fleet_energy[i][0] >= battery.energy_min, f"energy_min_constraint_{i}_{0}")
        self.model.Add(self.fleet_energy[i][0] <= battery.energy_max, f"energy_max_constraint_{i}_{0}")
        ...
        for t in range(1, self.n_t):
            ...
            self.model.Add(self.fleet_energy[i][t] >= battery.energy_min, f"energy_min_constraint_{i}_{t}")
            self.model.Add(self.fleet_energy[i][t] <= battery.energy_max, f"energy_max_constraint_{i}_{t}")
        ...
        if not battery.stationary:
            if not self.fully_charged_as_penalty:
                ...
                self.model.Add(self.fleet_energy[i][self.n_t - 1] >= battery.energy_end,
                ...
                remaining_energy_not_charged = battery.energy_end - self.fleet_energy[i][self.n_t - 1]
                ...
                self.cost["fully_charged_penalty"].append(remaining_energy_not_charged * self.fully_charged_penalty)
        ...
        for t in range(self.n_t):
            ...
            if battery.power_discharge_max == 0:
                self.model.Add(self.fleet_bool[i]["discharge"][t] == 0,
            ...
            if not battery.connected[t]:
                self.model.Add(self.fleet_bool[i]["disconnected"][t] == 1,
            ...
            # - if 1 at t-1 then only 1 is possible, so the vehicle is permanently disconnected
                ...
                self.model.Add(self.fleet_bool[i]["disconnected"][t] == 0,
                               f"disconnected_constraint_{i}_{t}")
                first_time_connected = False
            elif self.single_continuous_session_allowed:
                self.model.Add(self.fleet_bool[i]["disconnected"][t] >= self.fleet_bool[i]["disconnected"][t - 1],
                               f"disconnected_constraint_{i}_{t}")
    ...
    def _initialize_fleet_power(self):
        ...
        for t in range(self.n_t):
            ...
            source = self.grid_power["purchase"][t] * self.purchase_efficiency + self.model.Sum(
                [self.fleet_power[battery.id]["discharge"][t] * battery.efficiency_discharge
                 for battery in self.batteries])

            batteries_charge = [self.fleet_power[battery.id]["charge"][t] / battery.efficiency_charge
                                for battery in self.batteries]
            sink = self.grid_power["feed"][t] / self.feed_efficiency + self.grid_power['curtail'][t] + self.model.Sum(
                batteries_charge)
        ...
        for t in range(self.n_t):
            self.model.Add(
                self.grid_bool["feed"][t] + self.grid_bool["curtail"][t] + self.grid_bool["purchase"][t] == 1,
        ...
        for t in range(self.n_t):
            ...
            self.sum_power_fleet["discharge"][t] = self.model.Sum(
                [self.fleet_power[battery.id]["discharge"][t] for battery in self.batteries])
    ...
    def _calc_site_limits(self):
        ...
        if self.limit_as_penalty:
            ...
            if self.site_load_restriction_charge is not None:
                ...
                self.site_constraint["purchase"] = self.model.NumVar(self.site_load_restriction_charge,
                                                                     self.model.infinity(),
                                                                     "site_constraint_purchase")
                # Add constraint that the grid imports should always be below the constraint
                for t in range(self.n_t):
                    self.model.Add(self.grid_power["purchase"][t] <= self.site_constraint["purchase"],
                ...
                self.cost["site_limit_purchase"] = [
                    (self.site_constraint["purchase"]
                     - self.site_load_restriction_charge) * self.limit_purchase_penalty,
                ]
        else:
            for t in range(self.n_t):
                if self.site_load_restriction_discharge is not None:
                    self.model.Add(self.grid_power["feed"][t] <=
                                   self.grid_bool["feed"][t] * self.site_load_restriction_discharge,
                                   f"maximum_limit_feed_in_{t}")
                if self.site_load_restriction_charge is not None:
                    self.model.Add(self.grid_power["purchase"][t] <=
                                   self.grid_bool["purchase"][t] * self.site_load_restriction_charge,
    ...
    def _calc_cost_prices(self):
        ...
        if self.tariffs_import is not None:
            cost_import = [(1 - mm[t]) *
                           (self.tariffs_import[t] * (self.grid_power["purchase"][t] - site_load_pos[t]) * self.dt)
                           for t in range(self.n_t)]
        else:
        ...
        if self.tariffs_export is not None:
            cost_export = [-(1 - mm[t]) *
                           (self.tariffs_export[t] * (self.grid_power["feed"][t] - site_load_neg[t]) * self.dt)
                           for t in range(self.n_t)]
        else:
            cost_export = [0 for _ in range(self.n_t)]

        self.cost['Spot'] = [self.model.NumVar(-self.model.infinity(), self.model.infinity(), name=f'cost_tariffs_{t}')
                             for t in range(self.n_t)]

        if (self.tariffs_export is not None) and (self.tariffs_import is not None):
            _ = [self.model.Add(self.cost['Spot'][t] == cost_import[t] + cost_export[t], f'const_tariff_var_{t}')
                 for t in range(self.n_t)]
        elif self.tariffs_export is not None:
            _ = [self.model.Add(self.cost['Spot'][t] == cost_export[t], f'const_tariff_var_{t}')
                 for t in range(self.n_t)]
        elif self.tariffs_import is not None:
            _ = [self.model.Add(self.cost['Spot'][t] == cost_import[t], f'const_tariff_var_{t}')
                 for t in range(self.n_t)]
    ...
    def _calc_capacity_prices(self):
        ...
        if self.capacity_tariffs_import is not None:
            self.grid_peak["purchase"] = self.model.NumVar(0, self.model.infinity(), "peak_purchase")
            for t in range(self.n_t):
                self.model.Add(self.grid_power["purchase"][t] <= self.grid_peak["purchase"],
                               f'constraint_peak_purchase_{t}')
            self.cost['Capacity_Purchase'] = [self.capacity_tariffs_import * self.grid_peak["purchase"], ]

        if self.capacity_tariffs_export is not None:
            self.grid_peak["feed"] = self.model.NumVar(0, self.model.infinity(), "peak_feed")
            for t in range(self.n_t):
                self.model.Add(self.grid_power["feed"][t] <= self.grid_peak["feed"],
                               f'constraint_peak_feed_{t}')

            self.cost['Capacity_Feed'] = [self.capacity_tariffs_export * self.grid_peak["feed"], ]
    ...
    def _calc_cost_marketed_volumes(self):
        ...
        diff_pos = [self.model.NumVar(0, self.model.infinity(), name=f'marketed_volumes_Diff_Pos_{t}')
                    for t in range(self.n_t)]
        diff_neg = [self.model.NumVar(0, self.model.infinity(), name=f'marketed_volumes_Diff_Neg_{t}')
                    for t in range(self.n_t)]
        ...
        for t in range(self.n_t):
            if self.mask_marketed[t]:
                ...
                diff[t] = self.marketed_volumes[t] - self.dt * (self.sum_power_fleet["charge"][t]
                                                                - self.sum_power_fleet["discharge"][t])
            else:
                diff[t] = 0
            self.model.Add(diff[t] == diff_pos[t] - diff_neg[t], f'marketed_volumes_diff_{t}')
        ...
        self.cost['MarketedVolumes'] = [10 * (diff_pos[t] + diff_neg[t]) for t in range(self.n_t)]
    ...
    def _calc_cost_spiky_behaviour(self):
        ...
        self.sum_power_fleet_diff = {t: {"increasing": self.model.NumVar(0, self.model.infinity(),
                                                                         name=f"diff_greater_{t}"),
                                         "decreasing": self.model.NumVar(0, self.model.infinity(),
                                                                         name=f"diff_smaller_{t}")
                                         } for t in range(self.n_t - 1)}

        # Computing the differences
        fleet_power_before = [self.model.Sum([self.fleet_power[battery.id]["charge"][t] - self.fleet_power[battery.id][
            "discharge"][t] for
                                              battery in self.batteries]) for t in range(self.n_t - 1)]
        fleet_power_after = [self.model.Sum([self.fleet_power[battery.id]["charge"][t + 1] - self.fleet_power[
            battery.id]["discharge"][t + 1] for
                                             battery in self.batteries]) for t in range(self.n_t - 1)]
        ...
        for t in range(self.n_t - 1):
            self.model.Add(fleet_power_diff[t] == self.sum_power_fleet_diff[t]["decreasing"] -
        ...
        spiky_costs = np.array([self.sum_power_fleet_diff[t]["decreasing"] + self.sum_power_fleet_diff[t][
            "increasing"] for t in range(self.n_t - 1)])
        self.cost["Spiky_Behaviour_Penalty"] = self.spiky_behaviour_penalty * spiky_costs
        return 0
    ...
    def _calc_battery_costs(self):
        ...
        if sum([bat.cycle_cost_per_kwh for bat in self.batteries]) == 0:
            print("WARNING: Trying to include battery cycle costs but the cycle costs are 0.Please use "
                  "Battery.add_cycle_costs()")
            logger.warning("Trying to include battery cycle costs but the cycle costs are 0. Please use "
                           "Battery.add_cycle_costs()")

        self.battery_costs = {bat.id: self.model.NumVar(0, self.model.infinity(), name=f'BatteryCost_{bat.id}')
                              for bat in self.batteries}

        for bat in self.batteries:
            i = bat.id
            logger.info(f"Battery {i}: Adding cycle costs of {bat.cycle_cost_per_kwh} €/kWh")
            print(f"Battery {i}: Adding cycle costs of {bat.cycle_cost_per_kwh} €/kWh")
            self.model.Add(self.battery_costs[i] == self.model.Sum([self.fleet_power[i]["charge"][t] +
                                                                    self.fleet_power[i]["discharge"][t]
                                                                    for t in range(
                    self.n_t)]) * self.dt * bat.cycle_cost_per_kwh)

        self.cost['battery_costs'] = [self.battery_costs[i] for i in self.battery_costs.keys()]
    ...
    def _calc_cost_triad(self):
        ...
        self.cost['Triad'] = [self.model.NumVar(-self.model.infinity(), self.model.infinity(),
                                                f"cost_triad_{t}") for t in range(self.n_t)]
        ...
        if self.site_load is not None:
            _ = [self.model.Add(
                self.cost['Triad'][t] == self.triad_tariffs_import[t] * (
                        self.grid_power["purchase"][t] - self.site_load[t]) * self.dt
                - self.triad_tariffs_export[t] * self.grid_power["feed"][t] * self.dt,
                f"constraint_triad_{t}") for t in range(self.n_t)]
        else:
            _ = [self.model.Add(
                self.cost['Triad'][t] == self.triad_tariffs_import[t] * self.grid_power["purchase"][t] * self.dt
                - self.triad_tariffs_export[t] * self.grid_power["feed"][t] * self.dt,
                f"constraint_triad_{t}") for t in range(self.n_t)]
    ...
    def _calc_pos_flex(self):
        ...
        for battery in self.batteries:
            ...
            # ---- Define Variables ----
            self.flex_pos[i] = [self.model.NumVar(0, self.model.infinity(),
                                                  name=f"pos_flex_{i}_{t}") for t in range(self.n_t)]
            ...
            for t in range(self.n_t):
                ...
                power_current = self.fleet_power[i]['charge'][t] / battery.efficiency_charge \
                ...
                power_discharge_max_0 = delta_energy_discharge_max_0 * battery.efficiency_discharge / self.dt
                power_discharge_max_1 = delta_energy_discharge_max_1 * battery.efficiency_discharge / self.dt

                # 3) Difference between max discharge and current charging power
                power_discharge_max_2 = battery.power_discharge_max * battery.efficiency_discharge + power_current

                self.model.Add(
                    self.flex_buffer * power_discharge_max_0 >= self.flex_pos[i][t], f'constraint_flex_pos_1_{i}_{t}'
                )
                self.model.Add(
                    self.flex_buffer * power_discharge_max_1 >= self.flex_pos[i][t], f'constraint_flex_pos_2_{i}_{t}'
                )
                self.model.Add(
                    power_discharge_max_2 >= self.flex_pos[i][t], f'constraint_flex_pos_3_{i}_{t}'
                )

        # Sum flex over all batteries
        self.flex_pos_total = {t: self.model.NumVar(- self.model.infinity(), self.model.infinity(),
                                                    name=f"total_pos_flex_{t}") for t in range(self.n_t)}

        for t in range(self.n_t):
            # TODO: add grid efficiencies if different than 1 (as in Ladepark Duisburg)
            self.model.Add(
                self.flex_pos_total[t] == sum([self.flex_pos[battery.id][t] for battery in self.batteries]),
    ...
    def _calc_neg_flex(self):
        ...
        for battery in self.batteries:
            ...
            # ---- Define Variables ----
            self.flex_neg[i] = [self.model.NumVar(0, self.model.infinity(),
                                                  name=f"neg_flex_{i}_{t}") for t in range(self.n_t)]
            ...
            for t in range(self.n_t):
                ...
                power_charge_max_0 = delta_energy_charge_max_0 / battery.efficiency_charge / self.dt
                power_charge_max_1 = delta_energy_charge_max_1 / battery.efficiency_charge / self.dt

                # 3) Difference between max charging power and current charging power
                power_charge_max_2 = battery.power_charge_max / battery.efficiency_charge - power_current

                self.model.Add(
                    self.flex_buffer * power_charge_max_0 >= self.flex_neg[i][t], f'constraint_flex_neg_1_{i}_{t}'
                )
                self.model.Add(
                    self.flex_buffer * power_charge_max_1 >= self.flex_neg[i][t], f'constraint_flex_neg_2_{i}_{t}'
                )
                self.model.Add(
                    power_charge_max_2 >= self.flex_neg[i][t], f'constraint_flex_neg_3_{i}_{t}'
                )

        # Sum flex over all batteries
        self.flex_neg_total = {t: self.model.NumVar(- self.model.infinity(), self.model.infinity(),
                                                    name=f"total_neg_flex_{t}") for t in range(self.n_t)}

        for t in range(self.n_t):
            # TODO: add grid efficiencies if different than 1 (as in Ladepark Duisburg)
            self.model.Add(
                self.flex_neg_total[t] == sum([self.flex_neg[battery.id][t] for battery in self.batteries]),
    ...
    def _calc_cost_pos_flex_forecast(self):
        ...
        self.cost['FlexPos'] = [- self.flex_pos_total[t] * self.prices_flex_pos[t] * self.mask_flex_pos[t]
                                for t in range(self.n_t)]
        return 0
    ...
    def _calc_cost_neg_flex_forecast(self):
        ...
        # Cost through prices, sum over time
        self.cost['FlexNeg'] = [- self.flex_neg_total[t] * self.prices_flex_neg[t] * self.mask_flex_neg[t]
                                for t in range(self.n_t)]
    ...
    def add_symmetrical_flex_constraints(self):
        ...
        """
        for t in range(self.n_t):
            self.model.Add(self.flex_pos_total[t] == self.flex_neg_total[t], f'constraint_symmetrical_flex_{t}')
    ...
    def _calc_cost_flex_matching(self):
        ...
        if self.marketed_flex_pos is not None:
            ...
            flex_pos_diff_pos = [self.model.NumVar(0, self.model.infinity(), name=f'Flex_Pos_Diff_Pos_{t}')
                                 for t in range(self.n_t)]
            flex_pos_diff_neg = [self.model.NumVar(0, self.model.infinity(), name=f'Flex_Pos_Diff_Neg_{t}')
                                 for t in range(self.n_t)]
            for t in range(self.n_t):
                if self.mask_flex_pos[t]:
                    # Both are in kW
                    flex_pos_diff[t] = self.flex_pos_total[t] - self.marketed_flex_pos[t]
                else:
                    flex_pos_diff[t] = 0
                self.model.Add(flex_pos_diff[t] == flex_pos_diff_pos[t] - flex_pos_diff_neg[t], f'Flex_Pos_Diff_{t}')
        ...
        if self.marketed_flex_neg is not None:
            ...
            flex_neg_diff_pos = [self.model.NumVar(0, self.model.infinity(), name=f'Flex_Neg_Diff_Pos_{t}')
                                 for t in range(self.n_t)]
            flex_neg_diff_neg = [self.model.NumVar(0, self.model.infinity(), name=f'Flex_Neg_Diff_Neg_{t}')
                                 for t in range(self.n_t)]
            ...
            for t in range(self.n_t):
                if self.mask_flex_neg[t]:
                    # Both are in kW
                    flex_neg_diff[t] = self.flex_neg_total[t] - self.marketed_flex_neg[t]
                else:
                    flex_neg_diff[t] = 0
                self.model.Add(flex_neg_diff[t] == flex_neg_diff_pos[t] - flex_neg_diff_neg[t], f'Flex_Neg_Diff_{t}')
    ...
    def _calc_objectives(self):
        ...
        if (self.triad_tariffs_export is not None) or (self.triad_tariffs_import is not None):
            self._calc_cost_triad()

        if (self.capacity_tariffs_export is not None) or (self.capacity_tariffs_import is not None):
        ...
        # Site Limits
        if (self.site_load_restriction_charge is not None) or (self.site_load_restriction_discharge is not None):
    ...
    def optimize(self):
        ...
        #  Initialize
        # ------------------------------------------------
        self.model = pywraplp.Solver("model", pywraplp.Solver.CBC_MIXED_INTEGER_PROGRAMMING)
        ...
        #  Optimizer
        # ------------------------------------------------
        logger.debug(f'Model: constraints = {self.model.NumConstraints()} | variables = {self.model.NumVariables()} \n'
        ...
        self.solve_time = pd.Timedelta(pd.Timestamp.now() - start_solving).total_seconds()
        self.objective_value = self.model.Objective().Value() if self.status == 0 else np.nan

        if self.status == 0:
            logger.debug(f'Problem solved | {self.model.wall_time()} milliseconds | '
        ...
        #  Parse Result
        # ------------------------------------------------
        result = SiteResult.create(self.__dict__, optimizer='or', default_strategy=self.default_strategy)
...
def main():
    ...
    battery1 = Battery(**{
        'id': 42, 'capacity': 40,
        'energy_start': 10, 'energy_end': 40,
        'energy_min': 5, "energy_max": 40,
        'charge_max': 5, 'discharge_max': 0,
        'connected': [False] * 7 + [True] * 18 + [False] * 5
    })

    battery2 = Battery(**{
        'id': 23, 'capacity': 40,
        'energy_start': 12, 'energy_end': 40,
        'energy_min': 5, "energy_max": 40,
        'charge_max': 5, 'discharge_max': 0,
    ...
    #
    fo.add_prices(tariffs_import=prices_import, tariffs_export=prices_import, capacity_tariffs_import=10)
    ...
    fo.add_site_limits(site_load_restriction_discharge=0, site_load_restriction_charge=20)
