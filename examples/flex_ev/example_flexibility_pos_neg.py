...
# Second Optimization: Add Flexibility Product
price_flex_pos = np.array([0] * 13 + [1] * 4 + [0] * 13)
fo.add_flex(
...
fo.add_prices(prices_import, prices_import)
# marketed_volumes = np.array([np.nan] * 5 + [4] * 5 + [np.nan] * 20)
# fo.add_marketed_volumes(marketed_volumes)
results2 = fo.optimize()

# -- Plotting --
plt.figure(figsize=(24, 24))
for i, (title, res, prices, prices_flex) in enumerate(
    zip(
        [
            "with DA",
            "w/ DA + Pos&Neg.Flex",
        ...
        [None, price_flex_pos, price_flex_pos],
    )
):
    ...
    if prices_flex is not None:
        plt.plot(prices_flex, label="Flex")
    plt.xlabel("Time")
    plt.ylabel("Prices [pound/kWh]")
    plt.legend()
    plt.suptitle("Differences Scenarios including Customer Wish", fontsize=26)

file_name = Path(__file__).parents[2] / Path("figures/flexev_flexibility_posneg.png")
